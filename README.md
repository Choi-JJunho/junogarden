...
- 🎨 **자연 테마** - 녹색 계열의 차분한 디자인
- ✨ **인터랙티브 배경** - Canvas 기반 파티클 애니메이션
...
```css
:root {
  --background: #f5f5f0;    /* 배경색 */
  --foreground: #1a1a1a;    /* 글자색 */
  --primary: #4a7c2c;       /* 주요 색상 (녹색) */
  --secondary: #6b9b3c;     /* 보조 색상 (밝은 녹색) */
  --accent: #8bc34a;        /* 강조 색상 (연한 녹색) */
}
```

다크 모드 색상은 `@media (prefers-color-scheme: dark)` 섹션에서 수정합니다.
